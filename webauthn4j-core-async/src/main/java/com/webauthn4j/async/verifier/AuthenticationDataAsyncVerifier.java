...
public class AuthenticationDataAsyncVerifier {
    ...
    private class AuthenticationDataVerification{
        ...
        public CompletionStage<AuthenticationData> execute(){
            return execStep1toStep14()
                    .thenCompose(unused -> execStep15toStep20())
                    .thenCompose(unused -> execStep21())
                    .thenCompose(unused -> execStep22toStep24())
                    .thenCompose(unused -> execStep25toStep27())
                    .thenApply(unused -> authenticationData);
        }
        ...
        private CompletionStage<Void> execStep1toStep14() {
            ...
            collectedClientData = authenticationData.getCollectedClientData();

            authenticatorData = authenticationData.getAuthenticatorData();
            serverProperty = authenticationParameters.getServerProperty();
            ...
            authenticator = authenticationParameters.getAuthenticator();
        ...
        private CompletionStage<Void> execStep22toStep24(){
            ...
            if (presentedSignCount > 0 || storedSignCount > 0) {
                ...
                //spec| fails the authentication ceremony or not, is Relying Party-specific.
                    ...
                    return maliciousCounterValueAsyncHandler.maliciousCounterValueDetected(authenticationObject);
                }
            }
            else {
                return CompletableFuture.completedFuture(null);
    ...
    public DefaultMaliciousCounterValueAsyncHandler getMaliciousCounterValueAsyncHandler() {
        return maliciousCounterValueAsyncHandler;
    }
    ...
    public List<CustomAuthenticationAsyncVerifier> getCustomAuthenticationAsyncVerifiers() {
        return customAuthenticationAsyncVerifiers;
    }
