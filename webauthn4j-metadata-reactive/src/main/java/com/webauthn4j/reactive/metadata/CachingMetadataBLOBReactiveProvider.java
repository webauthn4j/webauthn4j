...
public abstract class CachingMetadataBLOBReactiveProvider implements MetadataBLOBReactiveProvider {

    private final Logger logger = LoggerFactory.getLogger(CachingMetadataBLOBReactiveProvider.class);
    ...
    public @NotNull CompletionStage<MetadataBLOB> provide(){
        ...
        if(!needsMetadataBLOBUpdate(cachedMetadataBLOB, metadataBLOBLastUpdate)){
            CompletableFuture.completedFuture(cachedMetadataBLOB);
        }
        ...
        if(metadataBLOBRefreshingLock.tryLock()){
            doProvide()
                    ...
                    .exceptionally(e -> { logger.error("Failed to fetch MetadataBLOB.", e); return null;})
                    .whenComplete((__, e)-> metadataBLOBRefreshingLock.unlock());
        }
        return response;
    }
    ...
    static boolean needsMetadataBLOBUpdate(MetadataBLOB metadataBLOB, LocalDate metadataBLOBLastUpdate){
