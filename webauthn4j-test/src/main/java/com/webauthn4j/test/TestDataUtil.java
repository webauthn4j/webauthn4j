...
public class TestDataUtil {
    ...
    public static byte[] calculateSignature(PrivateKey privateKey, byte[] signedData) {
        try {
            Signature.SignatureBuilder signatureBuilder;
            if (privateKey.getAlgorithm().equals("EC")) {
                signatureBuilder = COSEAlgorithmIdentifier.ES256.signatureBuilder();
            } else {
                signatureBuilder = COSEAlgorithmIdentifier.RS256.signatureBuilder();
            }
            signatureBuilder.privateKey(privateKey);
            signatureBuilder.data(signedData);
