...
public class TestDataUtil {
    ...
    public static byte[] calculateSignature(PrivateKey privateKey, byte[] signedData) {
        try {
            com.webauthn4j.signature.Signature.Signer signer;

            if (privateKey.getAlgorithm().equals("EC")) {
                signer = COSEAlgorithmIdentifier.ES256.getSignatureSigner();
            } else {
                signer = COSEAlgorithmIdentifier.RS256.getSignatureSigner();
            }
