 ...
 */
class AttestationReactiveVerifier {
    ...
    private final List<AttestationStatementReactiveVerifier> attestationStatementReactiveVerifiers;

    private final CertPathTrustworthinessReactiveVerifier certPathTrustworthinessReactiveVerifier;
    private final SelfAttestationTrustworthinessReactiveVerifier selfAttestationTrustworthinessReactiveVerifier;

    // ~ Constructor
    // ========================================================================================================

    AttestationReactiveVerifier(
        ...
        this.attestationStatementReactiveVerifiers = attestationStatementReactiveVerifiers;
        this.certPathTrustworthinessReactiveVerifier = certPathTrustworthinessReactiveVerifier;
        this.selfAttestationTrustworthinessReactiveVerifier = selfAttestationTrustworthinessReactiveVerifier;
    }
            ...
            switch (attestationType) {
                ...
                case SELF:
                    if (attestationStatement instanceof CertificateBaseAttestationStatement) {
                        ...
                        return selfAttestationTrustworthinessReactiveVerifier.verify(certificateBaseAttestationStatement);
                    }
                    else {
                ...
                case ATT_CA:
                    if (attestationStatement instanceof CertificateBaseAttestationStatement) {
                        ...
                        return certPathTrustworthinessReactiveVerifier.verify(aaguid, certificateBaseAttestationStatement, registrationObject.getTimestamp());
                    }
                    else {
                        throw new IllegalStateException();
                    }
    ...
    private @NotNull CompletionStage<AttestationType> verifyAttestationStatement(@NotNull CoreRegistrationObject registrationObject) {
        return CompletionStageUtil.compose(()->{
            for (AttestationStatementReactiveVerifier verifier : attestationStatementReactiveVerifiers) {
                if (verifier.supports(registrationObject)) {
                    return verifier.verify(registrationObject);
                }
            }
            throw new BadAttestationStatementException(String.format("AttestationReactiveVerifier is not configured to handle the supplied AttestationStatement format '%s'.", registrationObject.getAttestationObject().getFormat()));
        });
    }
}
