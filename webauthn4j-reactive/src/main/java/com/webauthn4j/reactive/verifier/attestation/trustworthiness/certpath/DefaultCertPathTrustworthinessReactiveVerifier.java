...
public class DefaultCertPathTrustworthinessReactiveVerifier implements CertPathTrustworthinessReactiveVerifier {

    private final TrustAnchorReactiveRepository trustAnchorReactiveRepository;

    private boolean fullChainProhibited = false;

    public DefaultCertPathTrustworthinessReactiveVerifier(TrustAnchorReactiveRepository trustAnchorReactiveRepository) {
        this.trustAnchorReactiveRepository = trustAnchorReactiveRepository;
    }
    ...
    public CompletionStage<Void> verify(@NotNull AAGUID aaguid, @NotNull CertificateBaseAttestationStatement attestationStatement, @NotNull Instant timestamp) {
        return CompletionStageUtil.compose(()->{
            ...
            if(attestationStatement instanceof FIDOU2FAttestationStatement){
                ...
                return trustAnchorReactiveRepository.find(subjectKeyIdentifier);
            }
            else {
                return trustAnchorReactiveRepository.find(aaguid);
            }
    ...
    private TrustAnchor verifyCertPath(CertPath certPath, Set<TrustAnchor> trustAnchors, Instant timestamp){
        ...
        // revocationCheckEnabled flag is intentionally removed from DefaultCerPathTrustworthinessReactiveVerifier
