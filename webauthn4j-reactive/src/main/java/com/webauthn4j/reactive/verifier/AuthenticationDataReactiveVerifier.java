...
import com.webauthn4j.verifier.*;
...
public class AuthenticationDataReactiveVerifier {

    private final AssertionSignatureVerifier assertionSignatureVerifier = new AssertionSignatureVerifier();
    private final ClientExtensionReactiveVerifier clientExtensionReactiveVerifier = new ClientExtensionReactiveVerifier();
    private final AuthenticatorExtensionReactiveVerifier authenticatorExtensionReactiveVerifier = new AuthenticatorExtensionReactiveVerifier();

    private final List<CustomAuthenticationReactiveVerifier> customAuthenticationReactiveVerifiers;

    private OriginVerifier originVerifier = new OriginVerifierImpl();
    private DefaultMaliciousCounterValueReactiveHandler maliciousCounterValueReactiveHandler = new DefaultMaliciousCounterValueReactiveHandler();

    private boolean crossOriginAllowed = false;

    public AuthenticationDataReactiveVerifier(@NotNull List<CustomAuthenticationReactiveVerifier> customAuthenticationReactiveVerifiers) {
        AssertUtil.notNull(customAuthenticationReactiveVerifiers, "customAuthenticationReactiveVerifiers must not be null");
        this.customAuthenticationReactiveVerifiers = customAuthenticationReactiveVerifiers;
    }

    public AuthenticationDataReactiveVerifier() {
        this.customAuthenticationReactiveVerifiers = new ArrayList<>();
    }
    ...
    private class AuthenticationDataVerification{
        ...
        public CompletionStage<AuthenticationData> execute(){
            return execStep1toStep20()
                    .thenCompose((__)-> execStep21())
                    .thenCompose((__)-> execStep22toStep24())
                    .thenCompose((__)-> execStep25toStep27())
                    .thenApply((__)-> authenticationData);
        }

        private CompletionStage<Void> execStep1toStep20(){
            ...
            authenticatorData = authenticationData.getAuthenticatorData();
        ...
        private CompletionStage<Void> execStep21(){
            ...
            AuthenticationExtensionsClientOutputs<AuthenticationExtensionClientOutput> clientExtensions = authenticationData.getClientExtensions();
            AuthenticationExtensionsAuthenticatorOutputs<AuthenticationExtensionAuthenticatorOutput> authenticatorExtensions = authenticatorData.getExtensions();
            return clientExtensionReactiveVerifier.verify(clientExtensions).thenCompose((__void)-> authenticatorExtensionReactiveVerifier.verify(authenticatorExtensions));
        }
        ...
        private CompletionStage<Void> execStep22toStep24(){
            ...
            if (presentedSignCount > 0 || storedSignCount > 0) {
                ...
                //spec| fails the authentication ceremony or not, is Relying Party-specific.
                    ...
                    return maliciousCounterValueReactiveHandler.maliciousCounterValueDetected(authenticationObject);
                }
            }
            else {
                return CompletableFuture.completedFuture(null);
        ...
        private CompletionStage<Void> execStep25toStep27(){
            ...
            updateRecord(authenticationParameters.getAuthenticator(), authenticatorData);
            ...
            for (CustomAuthenticationReactiveVerifier customAuthenticationReactiveVerifier : customAuthenticationReactiveVerifiers) {
                customAuthenticationReactiveVerifier.verify(authenticationObject);
            }
            Iterator<CustomAuthenticationReactiveVerifier> iterator = customAuthenticationReactiveVerifiers.iterator();
            CompletableFuture<Void> completableFuture = CompletableFuture.completedFuture(null);
            while(iterator.hasNext()){
                CustomAuthenticationReactiveVerifier customAuthenticationReactiveVerifier = iterator.next();
                completableFuture = completableFuture.thenAccept((______void)-> customAuthenticationReactiveVerifier.verify(authenticationObject));
            }
