...
class CustomOriginVerifierTest {
    ...
    @BeforeEach
    void setup() {
        target.getRegistrationDataVerifier().setOriginVerifier(new CustomOriginVerifier());
    }
    ...
    void registration_test() {
        ...
        assertThatThrownBy(() -> target.verify(registrationData, registrationParameters)).isInstanceOf(BadOriginException.class);

    }

    private static class CustomOriginVerifier implements OriginVerifier {
        @Override
