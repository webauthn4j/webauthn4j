 ...
 */
@Deprecated
public class TPMISTAttestDeserializer extends StdDeserializer<TPMISTAttest> {

    public TPMISTAttestDeserializer() {
        super(TPMISTAttest.class);
    }

    @Override
    public TPMISTAttest deserialize(@NotNull JsonParser p, @NotNull DeserializationContext ctxt) {
        byte[] value = ctxt.readValue(p, byte[].class);
        try {
            return TPMISTAttest.create(value);
        } catch (IllegalArgumentException e) {
            throw new InvalidFormatException(p, "value is out of range", value, TPMISTAttest.class);
        }
    }
}
