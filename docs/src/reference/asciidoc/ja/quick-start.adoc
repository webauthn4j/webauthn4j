...
* `origin` には WebAuthn による登録・認証セレモニーの時点でクライアントブラウザ（ユーザーエージェント）が認識していたことを期待する Origin を指定してください。ブラウザは認識した Origin を clientData に書き込み、その内容を含めて署名します。サーバー側では、clientData に記録された Origin が期待値と一致するかを照合することで、想定外のページや別オリジンから不正に起動されたセレモニー（フィッシングやフレーミング誘発）を防ぎます。
* `rpId` には WebAuthn による認証を提供するサイトの rpId を指定して下さい。rpId は資格情報のスコープを指定するパラメータです。 詳しくは https://www.w3.org/TR/webauthn-3/#relying-party-identifier[WebAuthnの仕様書のrpIdの項] を参照して下さい。
* `challenge` には発行した Challenge を指定して下さい。`challenge` はリプレイ攻撃を防ぐ為のパラメータです。 サーバー側で `challenge` としてランダムなバイト列を生成し、フロントエンド側で WebAuthn JS API を実行する際にパラメータとして指定して署名対象に含め、サーバー側で値の一致を検証することで、リプレイ攻撃からユーザーを防御することが出来ます。 発行した Challenge を検証時まで永続化しておくのは WebAuthn4J ライブラリ呼出側の責務です。セッションなどに格納しておくと良いでしょう。
* （オプション）`topOrigin(...)`/`topOrigins(...)`/`topOriginPredicate(...)` を使用することで、許容するtop originを指定することが出来ます。
** WebAuthn は同一オリジン制約とクリックジャッキング対策の観点から、原則としてクロスオリジンを許容しません。
** ただし、認証ウィジェットを別オリジンのサイトに埋め込む、といったユースケースでは、親ページを限定して許容したい場合があります。その場合は `topOrigin(...)`/`topOrigins(...)`/`topOriginPredicate(...)` で許容する親ページ（top-level ドキュメントの Origin）を明示してください。どうしても任意の親ページを許可する必要がある場合は `anyTopOrigin()` を使用できますが、攻撃面が広がるため慎重な運用が必要です。
** 例（特定の親ページのみ許可）:
+
[source,java]
----
...
カウンタについて詳しくは https://www.w3.org/TR/webauthn-3/#sign-counter[WebAuthnの仕様書のカウンタの項] を参照して下さい。

その後、認証済セッションを作成するなど、ユーザー認証成功時の処理を実施下さい。
検証に失敗した場合は、 `VerificationException` のサブクラスの例外が発生します。

=== Secure Payment Confirmation (SPC) の検証

https://www.w3.org/TR/secure-payment-confirmation/[Secure Payment Confirmation (SPC)] は、WebAuthnを決済認証向けに拡張するW3C仕様です。
通常のWebAuthnがユーザーとRelying Party（RP）間の認証を行うのに対し、SPCはさらに決済の詳細情報（金額・通貨・販売者）を認証に紐付け、ユーザーが特定の取引に同意したことの暗号的な証拠を提供します。

==== SPC認証フロー

SPC認証は標準WebAuthnといくつかの重要な点で異なります：

* クライアントは `navigator.credentials.get()` の代わりに `PaymentRequest` APIを使用
* `CollectedClientData` に取引詳細を含む `payment` フィールドが追加される
* `ClientData.type` が `"webauthn.get"` ではなく `"payment.get"` になる
* 第三者（販売者など）がRelying Party（決済事業者など）に代わって認証を開始できる

[plantuml,spc authentication sequence ja,svg]
....
@startuml
title Secure Payment Confirmation 認証フロー
autonumber
actor "ユーザー"
participant "認証器"
participant "クライアント\n(ブラウザ)" as Client
participant "販売者\n(フロントエンド)" as MerchantFE
participant "販売者\nサーバー" as MerchantBE
participant "Relying Party\n(決済事業者)" as RP
database "RPストレージ" as Storage

MerchantFE -> MerchantBE: 決済要求\n(カート情報)
MerchantBE -> RP: 認証要求\n(取引情報)
RP -> RP: チャレンジ生成
RP -> Storage: チャレンジ保存\n(トランザクションIDに紐付け)
RP -> MerchantBE: チャレンジ、クレデンシャルID\n(決済プロトコル経由、例: 3Dセキュア)
MerchantBE -> MerchantFE: SPCオプション\n(チャレンジ、クレデンシャルID、決済手段、金額、販売者)

MerchantFE -> Client: PaymentRequest.show()\n("secure-payment-confirmation"メソッド)
Client -> Client: 取引確認UX表示\n(金額、販売者、決済手段)
Client -> "ユーザー": 同意要求\n(決済情報を表示)
"ユーザー" -> Client: 取引を承認
Client -> Client: クライアントデータ生成\n(チャレンジ、オリジン、決済情報)
Client -> "認証器": クレデンシャル要求
"認証器" -> "ユーザー": ユーザー検証
"ユーザー" -> "認証器": 認証ジェスチャ
"認証器" -> "認証器": authenticatorDataと\nclientDataHashに署名
"認証器" -> Client: クレデンシャル応答\n(authenticatorData、署名など)
Client -> MerchantFE: PaymentResponse\n(authenticatorData、clientData、署名)
MerchantFE -> MerchantBE: SPCアサーション\n(authenticatorData、clientData、署名)
MerchantBE -> RP: アサーション転送\n(トランザクションID付き)
Storage -> RP: チャレンジ読み込み
RP -> RP: 署名、チャレンジ、オリジン、\n決済情報（金額、決済手段、販売者）を検証
...
SPC検証サポートは専用の `webauthn4j-spc` モジュールに含まれています。
...
SPC Registrationは標準WebAuthnの `navigator.credentials.create()` APIに `payment` 拡張を指定して行います。
Relying Party（決済事業者など）は通常、販売者サイト上の自身のiframe内でRegistrationを実行します。

サーバーは `PublicKeyCredentialCreationOptions` を `payment` 拡張を含む JSON として返します。
フロントエンドは `PublicKeyCredential.parseCreationOptionsFromJSON()` でJSONをパースし、
`navigator.credentials.create()` に渡します。

.フロントエンド: SPCクレデンシャルの登録
[source,javascript,caption="Example {counter:number}: "]
----
// サーバーからRegistrationオプションを取得
// サーバーは PublicKeyCredentialCreationOptions の JSON を返す:
// {
...
.サーバーサイド: SPC Registrationの検証
[source,java,caption="Example {counter:number}: "]
----
SPCManager spcManager = new SPCManager();

String registrationResponseJSON = "<registrationResponseJSON>"; /* フロントエンドから受信 */
RegistrationData registrationData;
try {
...
SPC Authenticationは販売者が `PaymentRequest` APIで `"secure-payment-confirmation"` メソッドを使って開始します。
ブラウザは決済情報をユーザーに表示する取引確認UXを表示します。

サーバーは `SecurePaymentConfirmationRequest` のデータ（`spcData`）と `PaymentDetailsInit`（`total`）を含む
JSON オブジェクトを返します。フロントエンドは base64url エンコードされたバイナリフィールドを `ArrayBuffer` に変換し、
`PaymentRequest` コンストラクタに渡します。

.フロントエンド: SPCによる認証
[source,javascript,caption="Example {counter:number}: "]
----
// サーバーからSPC認証オプションを取得
// サーバーは以下のような JSON を返す:
// {
//   "spcData": {
//     "credentialIds": ["AQIDBA..."],
//     "challenge": "dGVzdC1jaGFsbGVuZ2U",
//     "rpId": "bank.example",
//     "instrument": { "displayName": "銀行カード ****1234", "icon": "https://bank.example/card-art.png" },
//     "payeeName": "販売者ショップ",
//     "payeeOrigin": "https://merchant.example",
//     "timeout": 60000
//   },
//   "total": { "label": "合計", "amount": { "currency": "JPY", "value": "500" } }
// }
const response = await fetch("/spc/authenticationOptions");
const options = await response.json();

// base64urlエンコードされたフィールドをArrayBufferに変換
...
// paymentResponse.details は PublicKeyCredential
...
// サーバープロパティ
Origin origin = null /* set origin（販売者のオリジン）*/;
String rpId = null /* set rpId */;
Challenge challenge = null /* set challenge */;
Origin topOrigin = null /* set topOrigin（SPCが呼び出されたトップレベルオリジン）*/;
ServerProperty serverProperty = ServerProperty.builder()
        .origin(origin)
        .rpId(rpId)
        .challenge(challenge)
...
// SPC固有のパラメータ：期待する決済情報
SPCAuthenticationParameters authenticationParameters = new SPCAuthenticationParameters(
        serverProperty,
        credentialRecord,
        new PaymentCurrencyAmount("JPY", "500"),               // 期待する取引金額
        new PaymentCredentialInstrument("銀行カード ****1234",   // 期待する決済手段
                "https://bank.example/card-art.png"),
        "販売者ショップ",                                        // 期待する販売者名
        new Origin("https://merchant.example")                  // 期待する販売者オリジン
);
...
`SPCAuthenticationParameters` は `AuthenticationParameters` をSPC固有の期待値で拡張したクラスです。
簡易コンストラクタでは `userVerificationRequired` と `userPresenceRequired` がデフォルトで `true` に設定されます。
`SPCManager` は内部で以下を検証します：

* `ClientData.type` が `"payment.get"` であること
* `payment.rpId` が期待するRP IDと一致すること
* `payment.topOrigin` が期待するトップレベルオリジンと一致すること（`ServerProperty.topOriginPredicate` で検証。設定必須）
* `payment.total` が期待する取引金額と一致すること
* `payment.instrument` が期待する決済手段と一致すること
* `payment.payeeName` と `payment.payeeOrigin` が期待する値と一致すること（指定時）

NOTE: SPC認証では、`ServerProperty` に `topOrigin`（または `topOriginPredicate`）を設定する必要があります。SPCは常にクロスオリジンの決済コンテキストを伴うため、`SPCAuthenticationVerifier` がこの設定を必須としています。

検証に成功すれば、Relying Partyはユーザーが特定の取引内容に同意したことの暗号的な証拠を得たことになります。
...
https://www.w3.org/TR/webauthn-3/#sign-counter[WebAuthnの仕様書のカウンタの項] を参照して下さい。
