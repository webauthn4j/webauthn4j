...
認証デバイスの登録時に構成証明を検証する際は、`WebAuthnRegistrationRequest` を引数に
...
得られた `WebAuthnRegistrationData` のインスタンスを `WebAuthnManager#validate` メソッドに渡して実行してください。

`WebAuthnRegistrationRequest` のメンバー はフロントエンド側でWebAuthn JS APIを実行して取得した値となります。
何らかの方法でフロントエンド側からサーバー側に伝送し、指定してください。

`WebAuthnRegistrationParameters` は、`WebAuthnManager#validate` メソッドのもう一つの引数であり、
...
認証時にアサーションを検証する際は、`WebAuthnAuthenticationRequest` を引数に  `WebAuthnManager#validate` メソッドを
実行してください。`WebAuthnAuthenticationRequest` の
...
何らかの方法でフロントエンド側からサーバー側に伝送し、指定してください。
`WebAuthnManager#validate` メソッドのもう一つの引数である `WebAuthnAuthenticationParameters`  の
