...
全てのAttestationをサポートしつつ、外部ライブラリへの依存関係を最小限に抑えたポータブルなライブラリです。
...
NOTE: WebAuthn4Jを利用して開発されているSpring Security WebAuthnがFIDO2 Transport Binding ProfileのREST API実装を提供しており、
Spring Security WebAuthnのREST APIに対してFIDO2 Test Toolsを実行して検証を行っています。
