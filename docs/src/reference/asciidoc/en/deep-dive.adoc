...
==== SPC: webauthn4j-spc.jar

Provides server-side verification for https://www.w3.org/TR/secure-payment-confirmation/[W3C Secure Payment Confirmation (SPC)].

==== SPC-Async: webauthn4j-spc-async.jar

Provides async variant of SPC verification features.
...
If you would like to perform custom serialization or deserialization, you need to pass Jackson 3's `JsonMapper` and `CBORMapper` with custom serializers and deserializers registered to WebAuthn4J.
...
==== Adding WebAuthn extensions from external modules

WebAuthn4J allows external modules to define and add custom WebAuthn extensions without modifying webauthn4j-core.

Extension containers such as `AuthenticationExtensionsClientOutputs` store raw data internally as an `ObjectNode`, and each extension is lazily deserialized through deserializers registered in Jackson Modules.
External modules can add their own extensions by registering extension deserializers in their own Jackson Module and passing an `ObjectConverter` containing that Module to WebAuthn4J.

===== Adding a Client Extension Output (JSON)

The following example shows how to add a fictional extension `exampleExtension` from an external module.

Assume the extension is defined as follows:
...
Based on this definition, the Client Extension Output is returned as JSON like this:
...
Here are the steps to make this extension available in WebAuthn4J:

*1. Define the extension data class*
...
*2. Implement a deserializer extending `ExtensionClientOutputDeserializer`*

The deserializer receives the entire `ObjectNode` of `AuthenticationExtensionsClientOutputs` and extracts the value for its key (`"exampleExtension"`).
It returns `null` if the key is not present.
...
*3. Create a Jackson Module and register the deserializer*
...
*4. Create an `ObjectConverter` with this Module and pass it to `WebAuthnManager`*

`ObjectConverter` automatically adds `WebAuthnJSONModule` / `WebAuthnCBORModule` to the provided
`JsonMapper` / `CBORMapper`, so you only need to register your external module.
...
*5. Retrieve the extension type-safely via `getExtension(Class)`*
...
The same pattern applies to the following extension types:

[cols="1,2,2"]
|===
|Type |Base deserializer class |Registration target

|Client Extension Input
|`ExtensionClientInputDeserializer`
|Jackson Module on `JsonMapper`

|Authenticator Extension Output
|`ExtensionAuthenticatorOutputDeserializer`
|Jackson Module on `CBORMapper`

|Authenticator Extension Input
|`ExtensionAuthenticatorInputDeserializer`
|Jackson Module on `CBORMapper`
|===
...
`ObjectConverter` registers these modules automatically.

If you want to use WebAuthn4J’s serializer and deserializer outside of `ObjectConverter`, you can register these modules onto Jackson’s `ObjectMapper`.

When working with FIDO Metadata Service (MDS) types such as `AAID` and `AuthenticatorStatus`, `WebAuthnMetadataJSONModule` provides the corresponding serializers and deserializers.
Currently, metadata entry-point classes such as `MetadataBLOBFactory`, `LocalFilesMetadataStatementsProvider`, and `LocalFilesMetadataStatementsAsyncProvider` automatically register fallback serializers/deserializers with a warning if `WebAuthnMetadataJSONModule` is not explicitly registered.
In a future release, explicit `WebAuthnMetadataJSONModule` registration will be required.
To register the module, use `ObjectConverter.rebuildWithJSONModule`:
